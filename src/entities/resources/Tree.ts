...
export class Tree {
  ...
  private _createModel(size: number): void {
    ...
    // Bottom layer
    const bottomFoliageGeometry = new THREE.ConeGeometry(
      1.2 * size,
      2 * size,
      8
    );
    ...
    // Middle layer
    const middleFoliageGeometry = new THREE.ConeGeometry(
      1 * size,
      1.8 * size,
      8
    );
    const middleFoliage = new THREE.Mesh(
      middleFoliageGeometry,
      foliageMaterial
    );
    ...
    // Top layer
    const topFoliageGeometry = new THREE.ConeGeometry(
      0.7 * size,
      1.5 * size,
      8
    );
